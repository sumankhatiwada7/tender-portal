...
export async function acceptbid(req: any, res: any) {
    try {
        ...
        if (!bidexisted) {
            const payload: apitype = { message: "Bid not found", sucess: false }
            return res.status(404).json(payload);
        }
        if (!tinderexisted) {
        ...
        // fetch rejected bids BEFORE updateMany so we still have their businessIds
        const rejectedBids = await bid.find({ tenderId: tenderid, _id: { $ne: bidid } });
        await bid.updateMany({ tenderId: tenderid, _id: { $ne: bidid } }, { status: "rejected" });
        ...
        if (acceptedBusiness?.email) {
            const t = templates.bidAccepted(
                ...
                tinderexisted.title,
                String(bidexisted.amount),
                String(bidexisted._id)
            );
            await new Notifier(
                new EmailNotification(acceptedBusiness.email, t.html, t.subject)
            ).send();
        }
        ...
        for (const rejectedBid of rejectedBids) {
            ...
            if (rejectedBusiness?.email) {
                const t = templates.bidRejected(
                    ...
                    tinderexisted.title,
                    String(rejectedBid._id)
                );
                await new Notifier(
                    new EmailNotification(rejectedBusiness.email, t.html, t.subject)
                ).send();
            }
        }

        const payload: apitype = { message: "Bid accepted and tender awarded successfully", sucess: true }
        return res.status(200).json(payload);
...
export async function rejectbid(req: any, res: any) {
    try {
        const tenderid = req.params.tenderid;
        const bidid = req.params.bidid;
        const tinderexisted = await Tender.findById(tenderid);
        ...
        if (business?.email) {
            ...
            await new Notifier(
                new EmailNotification(business.email, t.html, t.subject)
            ).send();
        }

        const payload: apitype = { message: "Bid rejected successfully", sucess: true }
        return res.status(200).json(payload);
